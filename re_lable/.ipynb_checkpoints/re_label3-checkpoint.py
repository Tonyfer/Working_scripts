...
def re_label3(ref, target):
    ...
    permu = list(permutations(range(dices.shape[0])))
    

    sum_max = 0

    for per in permu:
        ind = [(i, per[i]) for i in range(len(per))]
        arr = [dices[i] for i in ind]
        if sum(arr) > sum_max:
            pairs = ind    
            sum_max = sum(arr)
